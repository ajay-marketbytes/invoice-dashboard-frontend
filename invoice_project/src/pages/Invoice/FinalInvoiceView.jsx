...
const FinalInvoiceView = () => {
  ...
  const displayInvoiceNumber = final_invoice_number || invoice_number;
  const totalInWords = numberToWords(Math.round(total_due));

  const handlePrintAndSave = async () => {
    try {
      // Mark as saved final invoice
      ...
      html2canvas(input, {
        ...
        .then((canvas) => {
          ...
          // Only add additional pages if there is remaining content
          while (heightLeft > 0) {
            pdf.addPage();
            position -= pageHeight;

            // Adjust the height of the image to fit the remaining content
            const remainingHeight = Math.min(heightLeft, pageHeight);
            pdf.addImage(imgData, "PNG", 0, position, imgWidth, remainingHeight);
            heightLeft -= pageHeight;
          }

          pdf.save(`Final_Invoice_${displayInvoiceNumber}.pdf`);
  ...
  return (
    <div className="flex flex-col items-center">
      <div className="w-[21cm] h-[29.7cm] mx-auto p-5 box-border font-sans" ref={contentRef}>
        ...
        <div className="flex justify-between mb-5">
          ...
          <div className="w-1/3 text-right ml-4">
            <div className="mb-4">
              <p><b>Invoice No:</b> {displayInvoiceNumber}</p>
